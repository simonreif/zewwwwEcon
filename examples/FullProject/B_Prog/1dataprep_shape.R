...
shape_data <- st_read(here("A_Orig/VG2500_KRS.shp"))
...
tourism_data <- read_delim(here("A_Orig/45412-03-02-4-B_flat.csv"), delim = ";", escape_double = FALSE, trim_ws = TRUE)
tourism_data <- tourism_data |>
  filter(
    substr(`1_Auspraegung_Code`, 1, 2) == "08" |
      substr(`1_Auspraegung_Code`, 1, 2) == "09"
  ) |>
  filter(Zeit == 2023) |>
  filter(`2_Auspraegung_Label` == "Insgesamt") |>
  filter(!is.na(GAST02__Gaesteuebernachtungen__Anzahl)) |>
  select(AGS = `1_Auspraegung_Code`, overnight_stays = GAST02__Gaesteuebernachtungen__Anzahl)
...
uniklinik_ags <- c("09162", "09562", "09663", "09362", "09761", "09564", "08311", "08221", "08421", "08416", "08222")

analysis_df$uniklinik <- ifelse(substr(analysis_df$AGS, 1, 5) %in% uniklinik_ags, 1, 0)

analysis_df$uniklinik <- factor(
  analysis_df$uniklinik,
  levels = c(0, 1),
  labels = c("No University Hospital", "University Hospital")
)

# Save intermediate dataset
saveRDS(analysis_df, here("C_Temp/analysis_df.rds"))
